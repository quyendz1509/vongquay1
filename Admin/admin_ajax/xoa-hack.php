...
if (isset($_POST) && $_POST) {
	if (isset($_SESSION['id'])) {
		...
		if ($thongtin['rules'] != $websetting['role_admin']) {
			...
			if (!$reset->listHackGame($id)) {
				$error  = array('status' => 0, 'messages' => 'Không tồn tại bản hack trên.');
			}else{
				$reset->deleteHack($id);
				$error  = array('status' => 99, 'messages' => 'Đã xóa bản hack <strong>'.$id.'</strong> thành công');
