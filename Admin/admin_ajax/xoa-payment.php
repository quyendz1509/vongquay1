...
if (isset($_POST) && $_POST) {
	if (isset($_SESSION['id'])) {
		...
		if ($thongtin['rules'] != $websetting['role_admin']) {
			...
			if (!$reset->check_payment($id)) {
				$error  = array('status' => 0, 'messages' => 'Không tồn tại ngân hàng trên');
			}else{
				$reset->delete_bank($id);
				$error  = array('status' => 99, 'messages' => 'Đã xóa ngân hàng <strong>'.$id.'</strong> thành công');
