...
if (isset($_POST) && $_POST) {
	if (isset($_SESSION['id'])) {
		...
		if ($thongtin['rules'] != $websetting['role_admin']) {
			...
			if (!$reset->check_product($id)) {
				$error  = array('status' => 0, 'messages' => 'Không tồn tại tài khoản trên.');
			}else{
				$reset->delete_product($id);
				$error  = array('status' => 99, 'messages' => 'Đã xóa tài khoản <strong>'.$id.'</strong> thành công');
